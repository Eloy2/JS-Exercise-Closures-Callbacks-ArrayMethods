...
function processSum(numberList, callback) {
  let list = numberList.reduce((accu, curr) => {
    return accu + curr;
  },0);
...
function getFullNames(runners) {
  let list = [];
  runners.forEach((i) =>{
    let fullName = `${i.last_name}, ${i.first_name}`;
    return list.push(fullName);
  });
  return list;
}
...
function firstNamesAllCaps(runners) {
  let list = [];
  runners.map((i) =>{
    let fullName = `${i.first_name}`;
    return list.push(fullName.toUpperCase());
  });
  return list;
}
...
function tallyUpDonations(runners) {
  return runners.reduce((accu, curr) => {
    return accu + curr.donation;
  },0);
}
